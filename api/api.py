...
def insertar():
    ...
    cur.execute(
        ...
        (discussion, discussion, userId, created,
         subject, msg, wordcount, charcount)
    )
...
def obtenerMensajesForos():
    ...
    for i in cur:
        res.append({"id": i[0], "discussion": i[1], "parent": i[2],
                   "userId": i[3], "created": i[4], "subject": i[5], "message": i[6]})
...
def userLogin():
    ...
    for i in cur:
        res.append({"id": i[0], "username": i[2],
                   "firstname": i[3], "lastname": i[4], "email": i[5]})
...
def insertarIntentoQuiz():
    ...
    cur.execute(
        ...
        (quiz, userid, layout, timestart, timefinish,
         timefinish, timefinish, sumgrades)
    )
...
def obtenerRespuestasLesson():
    ...
    for i in cur:
        res.append({"pageId": i[0], "prevPageId": i[1], "nextPageId": i[2], "qType": i[3], "contents": i[4],
                   "answerId": i[5], "jumpTo": i[6], "score": i[7], "answer": i[8], "response": i[9]})
    return json.dumps(res)
